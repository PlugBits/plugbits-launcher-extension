# Chrome Web Store 掲載文言 v2.0.0（アップデート申請用）
...
■ Excel Overlay（Pro）
kintone の一覧を Excel のような表形式で開いて、そのまま編集。
◦ セル編集・範囲選択・コピー＆ペースト・入力即置換
...
Free: ランチャー・WatchList・Record Pin・コマンドパレット・Excel Overlay（閲覧）
Pro（¥980/月）: Excel Overlay での編集・保存・一括操作
...
```
【v2.0 の新機能】
◦ デザインを全面刷新（ダークモード対応・視認性の高い新アイコン）
◦ WatchList しきい値通知: 件数が設定値を超えたらデスクトップ通知
◦ コマンドパレットのチートシート（? キー）と日付コマンド入力
◦ kintone ページ内クイックランチャードック（非表示可）
◦ クイック新規レコード（Shift+Alt+N）
◦ 英語 UI に対応
◦ Pro の 14日間無料トライアル（メールだけで開始・クレカ不要）
◦ Excel Overlay: 入力即置換・Delete で範囲クリア・未保存ガードなど操作性向上
◦ 多数の不具合修正と安定性向上
```
...
■ Excel Overlay (Pro)
Open any kintone list view as an Excel-like grid and edit in place.
◦ Cell editing, range selection, copy & paste, type-to-replace
◦ Smart date input (t = today, +3 = in 3 days, end = end of month, ...)
◦ Column layout presets, subtable editing, paging
◦ Free plan can use it as a read-only viewer
...
Free: launcher, WatchList, Record Pin, command palette, Excel Overlay (view only)
Pro (JPY 980/month): editing, saving and bulk operations in Excel Overlay
...
1. **新しい必須権限なし** — v2.0.0 で追加した `notifications` は optional_permissions のため、
...
```
This update adds an optional desktop-notification feature and a 14-day Pro trial.
- "notifications" is added as an OPTIONAL permission, requested at runtime only when
  the user sets a record-count threshold in the options page.
- The Pro trial sends the user's email address to our licensing API
